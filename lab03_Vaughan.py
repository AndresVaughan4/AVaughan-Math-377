...
my_flower = Table().with_columns( 'Number of petals', 6, 'Name', 'Iris')
my_flower
...
imdb_by_year = imdb.sort('Year',False)
imdb_by_year
...
average_20th_century_rating = np.average(imdb.where('Decade',are.below(2000)).column('Rating'))
average_21st_century_rating = np.average(imdb.where('Decade',are.above(1990)).column('Rating'))
...
proportion_in_20th_century = imdb.where('Decade',are.below(2000)).num_rows
proportion_in_21st_century = imdb.where('Decade',are.above(1990)).num_rows
...
Remainder = imdb.with_column('Year Remainder', imdb.column('Year')%2)
num_even_year_movies = Remainder.where("Year Remainder", are.equal_to(0))
...
pos_year = population.where("Population", are.above(6000000000))
year_population_crossed_6_billion = 2015 - pos_year.num_rows
...
aranged_farmers = farmers_markets_locations.sort('y', True)
northern_markets = aranged_farmers.take(np.arange(0, 5, 1))
northern_markets
